...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.enableGraduations == YES) {
        for (int i = 0; i<60; i++) {
            ...
            if (i%5 == 0 && i%15 == 0) {
                CGSize fontsize = [[NSString stringWithFormat:@"%d",i == 0?12:i/5] sizeWithAttributes:@{NSFontAttributeName:FontGothamBold(15)}];
                CGPoint P3 = CGPointMake(P1.x - fontsize.width/2, P1.y - fontsize.height/2);
                if (i == 0) {
                    P3 = CGPointMake(P1.x - fontsize.width/2, P1.y - fontsize.height/2 + 3);
                }
                else if (i == 30){
                     P3 = CGPointMake(P1.x - fontsize.width/2, P1.y - fontsize.height/2 - 2);
                }
                CATextLayer * text = [CATextLayer layer];
                text.frame           = CGRectMake(P3.x, P3.y, fontsize.width, fontsize.height);
                text.contentsScale   = [[UIScreen mainScreen] scale];
                text.string          = [NSString stringWithFormat:@"%d",i == 0?12:i/5];
                text.font            = (__bridge CFTypeRef)@"Gotham-Bold";
                text.fontSize        = 15;
...
//                [self.layer addSublayer:text];
    ...
    if (self.enableDigit == YES) {
        ...
        CGFloat markingDistanceFromCenter = rect.size.width/2.0f - self.digitFont.lineHeight/4.0f - 15 + _digitOffset;
        NSInteger offset = 4;
        
        for(unsigned i = 0; i < 12; i ++){
            if ((i + 1 ) % 3 == 0) {
                NSString *hourNumber = [NSString stringWithFormat:@"%@%d", [NSString stringWithFormat:@"%@", i+1<10 ? @" ": @""] , i + 1 ];
                CGSize fontsize = [hourNumber sizeWithAttributes:@{NSForegroundColorAttributeName: self.digitColor, NSFontAttributeName: self.digitFont}];
                ...
                [hourNumber drawInRect:CGRectMake(labelX - self.digitFont.lineHeight/2.0f,labelY - self.digitFont.lineHeight/2.0f,fontsize.width,fontsize.height) withAttributes:@{NSForegroundColorAttributeName: self.digitColor, NSFontAttributeName: self.digitFont}];
            }
        }
    }
}
